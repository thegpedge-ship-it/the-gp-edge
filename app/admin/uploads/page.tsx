  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={itemVariants} className="bg-[#090d16] bg-[radial-gradient(rgba(255,255,255,0.05)_1px,transparent_1px)] [background-size:16px_16px] border border-slate-800 p-8 shadow-2xl relative overflow-hidden rounded-3xl text-white">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-80 h-32 bg-teal-500/10 rounded-full blur-[80px] pointer-events-none" />
        <div className="relative z-10">
          <div
            className="border-2 border-dashed border-slate-800 hover:border-teal-500/60 hover:bg-slate-900/30 rounded-2xl p-10 text-center transition-all duration-300 cursor-pointer group"
            onClick={simulateUpload}
          >
            <svg
              className="w-12 h-12 text-slate-500 mx-auto mb-4 group-hover:scale-110 group-hover:text-teal-400 transition-all duration-300 animate-float"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <p className="text-base font-semibold text-slate-100 mb-1 group-hover:text-white transition-colors">Drop your CSV file here</p>
            <p className="text-sm text-slate-400 mb-6 font-light">or click to browse · Supports .csv files up to 10MB</p>
            <div className="flex items-center justify-center gap-3">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-950/80 border border-slate-800 rounded-lg text-xs font-semibold text-slate-400">
              ...
              <button className="px-4 py-1.5 bg-slate-900 border border-slate-800 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg text-xs font-semibold transition-all">
                Download Template
              </button>
            </div>
          </div>
          ...
          {uploadProgress !== null && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-teal-400">{Math.min(Math.round(uploadProgress), 100)}%</span>
              </div>
              <div className="h-2 bg-slate-950 rounded-full overflow-hidden">
                <motion.div
                  animate={{ width: `${Math.min(uploadProgress, 100)}%` }}
                  className="h-full bg-gradient-to-r from-teal-500 to-emerald-400 rounded-full shadow-[0_0_8px_rgba(20,184,166,0.4)]"
            ...
            <div className="divide-y divide-red-100">
              {validationErrors.map((err, i) => (
                <div key={i} className="px-6 py-3 flex items-start gap-4">
                  ...
                  <div>
                    <span className="text-xs font-semibold text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded">{err.field}</span>
