  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Active Templates", value: templates.filter((t) => t.status === "active").length, color: "text-teal-600 dark:text-teal-400", icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
          { label: "Total Fields", value: templates.reduce((sum, t) => sum + t.fields, 0), color: "text-green-600 dark:text-green-400", icon: "M4 6h16M4 10h16M4 14h16M4 18h16" },
          { label: "Total Usage", value: totalUsage.toLocaleString(), color: "text-emerald-600 dark:text-emerald-400", icon: "M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" },
          { label: "Avg Fields/Template", value: Math.round(templates.reduce((sum, t) => sum + t.fields, 0) / templates.length), color: "text-slate-600 dark:text-slate-400", icon: "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" },
        ].map((s) => (
          <div key={s.label} className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-white dark:border-slate-800 p-5 shadow-md shadow-slate-200/30 dark:shadow-none relative overflow-hidden group hover:shadow-lg hover:border-teal-200/60 dark:hover:border-teal-500/30 hover:-translate-y-0.5 transition-all duration-300">
            <div className="absolute inset-0 bg-gradient-to-br from-white/80 dark:from-slate-900/80 via-transparent to-teal-50/5 pointer-events-none" />
            <div className="relative z-10 flex items-start justify-between">
              <div>
                <p className={`text-2xl font-bold ${s.color} mb-0.5`}>{s.value}</p>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wider">{s.label}</p>
              </div>
              <div className={`p-2 rounded-xl bg-slate-50 dark:bg-slate-800 ${s.color} opacity-40`}>
      ...
      <motion.div variants={itemVariants} className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" placeholder="Search templates..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 text-sm bg-white/80 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 dark:text-slate-100 transition-all" />
        </div>
        {["all", "active", "draft", "suspended"].map((f) => (
          <button key={f} onClick={() => setStatusFilter(f)} className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-all ${statusFilter === f ? "bg-teal-50 text-teal-700 border-teal-200 dark:bg-teal-950/40 dark:text-teal-400 dark:border-teal-900/50" : "bg-white text-slate-500 border-slate-200 hover:border-slate-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700 dark:hover:border-slate-600"}`}>
          ...
          {filtered.map((template, idx) => {
            ...
            return (
              <motion.div
                ...
                className="group relative bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-2xl border border-white/80 dark:border-slate-800/80 shadow-md dark:shadow-none shadow-slate-200/40 overflow-hidden cursor-pointer hover:shadow-xl hover:shadow-teal-500/10 hover:border-teal-200/60 dark:hover:border-teal-500/30 hover:-translate-y-1 transition-all duration-300"
              >
