...
export default function QuizzesPage() {
  const { currentAdmin, isReadOnly, canRestoreItem, canArchiveItem, isSuperAdmin } = useAdminRole();
                        ...
                        <>
                          ...
                          {isSuperAdmin && (
                            <button
                              ...
                              title="Delete Quiz Permanently (SA Only)"
                            >
            ...
            >
              <div className="px-6 pt-6 pb-4 text-center">
                <div className={`w-14 h-14 mx-auto rounded-2xl flex items-center justify-center mb-4 border ${
                  quizToDelete.status === "archived"
                    ? "bg-rose-50 dark:bg-rose-950/20 border-rose-100 dark:border-rose-900/30 text-rose-500 dark:text-rose-400"
                    : "bg-amber-50 dark:bg-amber-950/20 border-amber-100 dark:border-amber-900/30 text-amber-500 dark:text-amber-400"
                }`}>
                  {quizToDelete.status === "archived" ? (
                    <Lucide.Trash2 className="w-7 h-7" />
                  ) : (
                    <Lucide.Archive className="w-7 h-7" />
                  )}
                </div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">
                  {quizToDelete.status === "archived" ? "Delete Mock Exam Permanently?" : "Archive Mock Exam?"}
                </h2>
                <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-1">
                  {quizToDelete.status === "archived"
                    ? "Are you sure you want to permanently delete this mock exam? This action is irreversible."
                    : "Are you sure you want to archive this mock exam? It can be restored later from the Archived filter."}
                </p>
              ...
              <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-[13px] font-semibold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors duration-200 cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  onClick={confirmDeleteQuiz}
                  className={`flex-1 px-4 py-2 rounded-lg text-white text-[13px] font-bold shadow-md transition-all duration-200 cursor-pointer ${
                    quizToDelete.status === "archived"
                      ? "bg-rose-600 hover:bg-rose-500 shadow-rose-600/20"
                      : "bg-amber-600 hover:bg-amber-500 shadow-amber-600/20"
                  }`}
                >
                  {quizToDelete.status === "archived" ? "Delete Permanently" : "Archive Exam"}
                </button>
              </div>
            </motion.div>
          </motion.div>
