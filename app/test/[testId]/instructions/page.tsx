...
export default function InstructionsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center px-4 py-6">
      ...
      >
        {/* Header — title + summary stats (stacked on mobile, one row on sm+) */}
        <div className="px-5 sm:px-8 py-4 border-b border-slate-200 dark:border-slate-700 bg-emerald-50/60 dark:bg-emerald-900/10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
          <div className="min-w-0">
            ...
            <h1 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-slate-100 truncate">{config.name}</h1>
          </div>
          <div className="flex items-center justify-between sm:justify-normal gap-4 sm:gap-8 flex-shrink-0">
        ...
        {/* Footer — declaration + start (stacked on mobile, one row on sm+) */}
        <div className="px-5 sm:px-8 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
          <label className="flex items-start sm:items-center gap-3 cursor-pointer select-none min-w-0">
            <input
              type="checkbox"
              checked={agreed}
              onChange={(e) => setAgreed(e.target.checked)}
              className="ui-checkbox mt-0.5 sm:mt-0"
            />
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto sm:flex-shrink-0">
            <button
              onClick={() => router.back()}
              className="flex-1 sm:flex-none px-4 py-2.5 sm:py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-[13px] font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors duration-200 whitespace-nowrap"
            >
              &larr; Go Back
            </button>
            <button
              ...
              disabled={!agreed}
              className={`flex-1 sm:flex-none px-7 py-2.5 sm:py-2 rounded-lg text-[14px] font-bold transition-all duration-200 whitespace-nowrap ${
                agreed
