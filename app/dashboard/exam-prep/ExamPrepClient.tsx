...
export default function ExamPrepClient({ initialQuizzes }: { initialQuizzes?: QuizItem[] }) {
  ...
  const [loading, setLoading] = useState(initialQuizzes === undefined);
  ...
  useEffect(() => {
    if (initialQuizzes !== undefined) return;
    let cancelled = false;
